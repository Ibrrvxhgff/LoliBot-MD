...
if (!text) return m.reply( `مثال على الاستخدام: ${usedPrefix + command} https://music.apple.com/us/album/glimpse-of-us/1625328890?i=1625328892`);
if (userRequests[m.sender]) {
conn.reply(m.chat, `⚠️ مهلاً @${m.sender.split('@')[0]}، أنت تقوم بالفعل بتنزيل أغنية 🙄\nانتظر حتى ينتهي التنزيل الحالي قبل طلب أغنية أخرى. 👆`, userMessages.get(m.sender) || m)
return;
}
...
if (!musicData || !musicData.success) throw new Error('تعذر الحصول على البيانات من واجهة برمجة تطبيقات appledown');
...
if (!songData)  throw new Error('تعذر تنزيل الأغنية من أي واجهة برمجة تطبيقات');
const texto = `*• العنوان:* ${songData.name}\n*• الفنانون:* ${songData.artists}\n*• المدة:* ${songData.duration}${songData.url ? `\n*• الرابط:* ${songData.url}` : ''}`;
...
handler.command = /^(applemusic|اغاني_ابل)$/i;
