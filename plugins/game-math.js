...
return m.reply(`⚠️ يجب عليك اختيار صعوبة صالحة.
...
return m.reply(`✅ صحيح! لقد ربحت *${exp} XP*`);
} else {
data.intentos--;
if (data.intentos <= 0) {
mathGames.delete(m.sender);
return m.reply(`❌ لقد أخطأت 3 مرات. الإجابة الصحيحة كانت *${result}*.`);
} else {
mathGames.set(m.sender, data);
return m.reply(`❌ غير صحيح. يتبقى لديك *${data.intentos}* محاولة.`);
}}
};
handler.help = ['رياضيات [صعوبة]'];
handler.tags = ['game'];
handler.command = ['math', 'mates', 'matemáticas', 'رياضيات'];
